...
public class Baek15683_new {
    ...
    static int[][][] cdir = {   // cdir[cctv번호][cctv방향][방향에따른 감시경로]
                            {{0}, {1}, {2}, {3}},                           // 1번 cctv
                            {{0, 1}, {2, 3}},                               // 2번
                            {{0, 3}, {3, 1}, {1, 2}, {2, 0}},               // 3번
    ...
    public static void bfs(int[] number) { // number = cctv 방향 달림
        for(int i = 0; i < list.size();i++) {
            ...
            for(int d = 0; d < cdir[current.status - 1][r].length; d++) { //cdir[list.get(i).status][number[i]][d]
                int nr = current.r;
                int nc = current.c;
